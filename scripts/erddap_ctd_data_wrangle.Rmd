...
combined_event_ctd <- sheet2 %>% 
  mutate(cruise_name = "GoA2019",
         ...
         zone = "NA",
         event_type = "CTD",
         station_event_ID = paste(cruise_name, zone, `NO.(ST)`, event_type, 
                                  sep = "-"),
         year = year(eventDate),
  ...
  select(cruise_name, vessel_name_abbr, zone, "station" = `NO.(ST)`, event_type,
   ...
   "sampling_depth_meters" = DEPTH,
    ...
    "sea_water_chl_fluorescence" = `Chl-Flu. [ppb]_R`,
    "sea_water_chl_a_concentration" = `Chl-a [ug/l]_R`,
    "sea_water_dissolvedO2" = `DO [mg/l]_R`,
    "sea_water_dissolvedO2_sat" = `DO [%]_R`,
    "sea_water_concentration_of_oxygen" = `O2 [ml/l]`,
    "sea_water_pH" = pH,
    "sea_water_BOD5" = `BOD5[ml/l]`
    ) 
